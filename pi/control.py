...
def pure_pursuit(current_x, current_y, current_heading, path, index):
    ...
    print(f"[DEBUG] Entering pure_pursuit - current_x: {current_x}, current_y: {current_y}, current_heading: {current_heading}, index: {index}")
    closest_point = None
    v = speed
    for i in range(index, len(path)):
        ...
        print(f"[DEBUG] Checking path point ({x}, {y}) - distance: {distance}")
    ...
    if closest_point is not None:
        ...
        print(f"[DEBUG] Closest point found - closest_point: {closest_point}, target_heading: {target_heading}, desired_steering_angle: {desired_steering_angle}")
    else:
        target_heading = math.atan2(path[-1][1] - current_y, path[-1][0] - current_x)
        desired_steering_angle = target_heading - current_heading
        index = len(path) - 1
        print(f"[DEBUG] No closest point found, using last path point - target_heading: {target_heading}, desired_steering_angle: {desired_steering_angle}")
    ...
    if desired_steering_angle > math.pi/6 or desired_steering_angle < -math.pi/6:
        ...
        desired_steering_angle = 0.0  # Zero out angular velocity if we're moving straight

    # Ensure only one type of motion is commanded
    if v != 0.0:
    ...
    # Debugging statement
    print(f"[DEBUG] Exiting pure_pursuit - v: {v}, w: {desired_steering_angle}, index: {index}")
        ...
        while True:
            ...
            if self.kesif == True:
                ...
                if v is None:
                    ...
                    print(f"[DEBUG] exp - pure_pursuit returned v: {v}, w: {w}, i: {self.i}")
